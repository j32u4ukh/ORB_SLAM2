...
namespace ORB_SLAM2
{
    ...
    Sim3Solver::Sim3Solver(KeyFrame *pKF1, KeyFrame *pKF2, const vector<MapPoint *> &vpMatched12, const bool bFixScale) : mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale)
    {
        ...
        // 利用『相機內參 mK2』將『空間點 mvX3Dc2』由世界座標轉換到『成像平面座標 mvP2im2』
    ...
    cv::Mat Sim3Solver::iterate(int nIterations, bool &bNoMore, vector<bool> &vbInliers, int &nInliers)
    {
        ...
        while (mnIterations < mRansacMaxIts && nCurrentIterations < nIterations)
        {
            ...
            // 利用『相似轉換矩陣』和『相機內參』將『空間點』投影到『成像平面座標』，計算與原本的成像座標的誤差，
            // 判斷哪些是內點
            CheckInliers();
    ...
    void Sim3Solver::ComputeSim3(cv::Mat &P1, cv::Mat &P2)
    {
        ...
        // ComputeCentroid(P, Pr, C) C 為 P 每一列的平均，Pr 為 P 各列分別減掉列平均（即 C）
    ...
    void Sim3Solver::CheckInliers()
    {
        ...
        // 利用『相似轉換矩陣 mT12i』和『相機內參 mK1』將『空間點 mvX3Dc2』投影到『成像平面座標 vP2im1』
        Project(mvX3Dc2, vP2im1, mT12i, mK1);

        // 利用『相似轉換矩陣 mT21i』和『相機內參 mK2』將『空間點 mvX3Dc1』投影到『成像平面座標 vP1im2』
